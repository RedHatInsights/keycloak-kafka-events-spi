...
* AdminEventProvider to record the internal Keycloak admin events and push to Kafka Topic
...
### Enable events-listener event

- To enable the event listener via the GUI interface, go to _Manage -> Events -> Config_. The _Event Listeners_ configuration should have an entry named `events-listener`.
